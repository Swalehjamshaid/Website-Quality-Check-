    ...
    with app.app_context():
        ...
        try:
            ...
            audit = Audit(
                ...
                fid=45, cls=0.04, fcp=round(load_time * 0.9, 2), tbt=200,
                seo_score=95 if soup.title else 40,
                performance_score=90, accessibility_score=94, best_practices_score=92,
                ...
                broken_links=0,
                internal_links=len(links) // 2,
                external_links=len(links) // 2,
                ...
                unused_css=18.5, unused_js=42.3,
                render_blocking=10, third_party_requests=15,
...
@login_required
def site_detail(wid):
    return f"Site Detail Placeholder {wid}"

@app.route('/report/<int:wid>')
@login_required
def report(wid):
    return f"Report Placeholder {wid}"
